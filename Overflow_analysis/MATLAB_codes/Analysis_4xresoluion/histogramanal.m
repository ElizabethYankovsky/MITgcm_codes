...
%Initial:
Si=sq(ncread('state2.nc','S',[1 1 1 40],[Inf Inf Inf 1]));
Ti=sq(ncread('state2.nc','Temp',[1 1 1 40],[Inf Inf Inf 1]));

densityi = densmdjwf(Si,Ti,zeros(2560,480));
%Last time step:
Sf=sq(ncread('state3.nc','S',[1 1 1 80],[Inf Inf Inf 1]));
Tf=sq(ncread('state3.nc','Temp',[1 1 1 80],[Inf Inf Inf 1]));
densityf = densmdjwf(Sf,Tf,zeros(2560,480));


myD = ncread('grid.nc','Depth');
[hfacC,ddz]=hfac(dz',-myD,0.05,0);
volume=zeros(2560,480); %FIND V of each grid cell
for i =1:2560
    for j = 1:480
        volume(i,j) = dy*dx(i)*ddz(i,j);
    end
end
volume1=volume(1:680,:); volume2=volume(681:2000,:); 
density1i=densityi(1:680,:); density2i=densityi(681:2000,:); 
density1f=densityf(1:680,:); density2f=densityf(681:2000,:); 
