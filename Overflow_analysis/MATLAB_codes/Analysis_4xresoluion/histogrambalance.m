...
SF = reshape(SF,[2560 1]);
SF = squeeze(SF);


forcing = -SF*(7.7*10^-4)+Q*(3.2*10^-5)/(3974); %kg/(m^2*s)
forcingarea=dx*dy; %m^2 INTEGRATED OVER Y, NOT OVER X
time = 30*24*3600;%s
totalfx=forcing.*forcingarea*time; %kg

source1=sum(totalfx(1:680)); %kg
source2=sum(totalfx(681:2000)); %kg
...
ylim([bins(1) bins(end)])
ylabel('Density (kg/m^3)');
%xlabel('Percentage of Region 2 Volume');
