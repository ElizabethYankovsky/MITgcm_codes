...
% Flux
Qo=500; %W/m^2
SFo=0.0372; %kg*psu/(m^2*s);

% Stratification
gravity=9.81;
talpha=2.0e-4;
sbeta=7.4e-4;
N2=0.0000013; %0 for no stratification% save Sweighted.mat Sweighted
% save Tweighted.mat Tweighted
% save densweighted.mat densweighted
% save Uweighted.mat Uweighted
% save densityanomaly.mat densityanomaly
% save XC.mat XC
% save YC.mat YC
...
slope=0.15;
offset=repmat(1.5e3 + Lx/2.0,1,ny);
amplitude = offset(1)/exp(-1.4*(y(300)-Ly/2).^2/(Ly/3).^2)-offset(1,1);
for j = 300:600
    offset(1,j) = offset(1,1)+(sin((j-375)*2*pi/300)+1)/2 *amplitude;
end
% for j=300:600
%     offset(1,j)=amplitude*exp(-1.4*(y(j)-Ly/2).^2/(Ly/3).^2);
% end
dmax=-40.0;
h1=dmax;
h2=-H;
...
for i=1:nx
for j=1:ny
d(i,j) = hdiff/2*(tanh((Lx-x(i)-offset(j))/xwidth) + 1) - H;
end
end
d(1,:)=0.0;
